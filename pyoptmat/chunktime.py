...
def dot(A, v):
    ...
    nblk = A.shape[0] 
    sblk = A.shape[2]
    vp = v.view(sbat, nblk, sblk).transpose(0,1)
    
    b = torch.bmm(A.view(-1,sblk,sblk),
            vp.reshape(sbat*nblk,sblk).unsqueeze(-1)).squeeze(-1).view(nblk,sbat,sblk)
    b[1:] -= vp[:-1]

    return b.transpose(1,0).flatten(start_dim=1)

@torch.jit.script
def inv_dot(lu, pivots, b):
...
@torch.jit.script
def gmres_direct(A, b, lu, pivots, tol : float = 1e-10, 
        maxiter : int = 100, check : int = 5, eps : float = 1.0e-15):
    """
    ...
    for k in range(maxiter):
        # A \cdot v
        w = dot(A, Z[..., :, k].unsqueeze(-1)).squeeze(-1)
        ...
        if (k + 1) % check == 0 or (k + 1) == maxiter:
            ...
            r = b - dot(A, x.unsqueeze(-1)).squeeze(-1)
...
class BackwardEulerLUFactorization(BackwardEulerOperator):
    """
    Manages the data needed to solve our special system via LU factorization
    ...
    def _setup_factorization(self):
        ...
        Form the factorization...
        """
        factorization = torch.zeros_like(self.diag)
        self.operator = torch.zeros((self.sbat, self.nblk, self.sblk, self.nblk, self.sblk), device = self.device, dtype = self.dtype)
        ...
        for i in range(self.nblk):
            ...
            factorization[i] = torch.bmm(factorization[i], self.diag[i])
            if i != self.nblk - 1:
...
class ChunkTimeOperatorSolverContext:
    ...
    def __init__(self, solve_method, gmres_reuse_iters = 50,
            ...
            gmres_check = 5, factorization = BackwardEulerThomasFactorization):
    ...
    def solve_gmres(self, J, R):
        ...
        if (
                self.lu is None or 
                self.lu.shape != J.shape or
                self.last_iters > self.gmres_reuse_iters
            ):
            self.lu, self.pivots, _ = torch.linalg.lu_factor_ex(J) 
        
        b, self.last_iters = gmres_direct(J, R, self.lu, self.pivots,
                tol = self.gmres_tol,
