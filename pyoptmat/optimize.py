...
class StatisticalModelExperiment(PyroModule):
  ...
  def forward(self, exp_data, exp_cycles, exp_types, exp_inds, exp_results = None):
    """
      Integrate forward and return the result
    ...
    """
    model = self.maker(*self.get_params())
    predictions = model.solve_both(exp_data[0], exp_data[2], exp_data[1], exp_inds)
    results = experiments.assemble_results_full(exp_data, exp_cycles, exp_types, predictions[:,:,0])
    with pyro.plate("trials", exp_data.shape[2]):
      with pyro.plate("time", exp_data.shape[1]):
        return pyro.sample("obs", dist.Normal(results, self.eps), obs = exp_results)
...
class HierarchicalStatisticalModelExperiment(PyroModule):
  ...
  def make_guide(self):
    """
      ...
      is going to need
    """
    def guide(exp_data, exp_cycles, exp_types, exp_inds, exp_results = None):
      # Setup and sample the top-level loc and scale
      top_loc_samples = []
      top_scale_samples = []
      for var, loc_loc, loc_scale, scale_scale, in zip(
  ...
  def forward(self, exp_data, exp_cycles, exp_types, exp_inds, exp_results = None):
    ...
    with pyro.plate("trials", exp_data.shape[2]):
      ...
      predictions = bmodel.solve_both(exp_data[0], exp_data[2], exp_data[1], exp_inds)
      results = experiments.assemble_results_full(exp_data, exp_cycles, exp_types, predictions[:,:,0])

      # Sample!
      with pyro.plate("time", exp_data.shape[1]):
        pyro.sample("obs", dist.Normal(results, eps), obs = exp_results)
