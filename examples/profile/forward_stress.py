...
if __name__ == "__main__":
  ...
  stresses = np.zeros((ntime,nbatch))

  for i in range(nbatch):
    cycle = experiments.generate_random_cycle(R = [-1,-0.99], max_strain = [50.0,200.0])
    times[:,i], stresses[:,i] = experiments.sample_cycle_normalized_times(cycle, N, 
        nload = nload, nhold = nhold)
  
  times = torch.tensor(times, device = device)
  stresses = torch.tensor(stresses, device = device)
  ...
  ts = time.time()
  with torch.no_grad():
    res = model.solve_stress(times, stresses, temperatures)
  tt = time.time() - ts
