...
if __name__ == "__main__":
  ...
  times, true_strains, temperatures, stresses = load_data(scale, nsamples, device = device)

  # 2) Setup names for each parameter and the initial conditions
  names = ["n", "eta", "s0", "R", "d"]
  ics = [ra.uniform(0.1,0.9) for i in range(len(names))]
  #ics = [0.99,0.60,0.45,0.17,0.94]
  ...
  def closure():
    optim.zero_grad()
    pred = model(times, stresses, temperatures, mode = "stress")
    lossv = loss(pred, true_strains)
    lossv.backward()
    return lossv
