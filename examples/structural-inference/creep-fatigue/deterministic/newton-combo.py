...
if __name__ == "__main__":
    ...
    for scale in scales:
        for sample in samples:
            ...
            scale = scale
            nsamples = sample  # 10 is the full number of samples in the default dataset
            ...
            np.savetxt("loss_newton_{}_{}.txt".format(sample, scale), loss_history)
            plt.figure()
            plt.plot(loss_history)
            plt.xlabel("Iteration")
            plt.ylabel("Loss")
            plt.tight_layout()
            plt.savefig("convergence_newton_{}_{}.png".format(sample, scale), dpi=300)
