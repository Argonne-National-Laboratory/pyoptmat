...
eta_true = 300.0
s0_true = 50.0
C_true = np.array([10000.0, 1000.0, 100.0])
g_true = np.array([100.0, 50.0, 500.0])
...
if __name__ == "__main__":
    ...
    max_strains = np.linspace(0.0025, 0.005, 5)
    # Tension hold in the cycle
    tension_holds = np.array([1.0e-6, 1.0, 10.0, 100.0, 1000.0])
    ...
    temperature = 0.0  # Problem will be temperature independent

    # Discretization
    N = 20  # Number of cycles
    nsamples = 10  # Number of repeats of each test
    nload = 10  # Time steps during the load period of each test
    nhold = 10  # Time steps during the hold period of each test
