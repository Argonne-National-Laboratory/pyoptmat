...
if __name__ == "__main__":
    ...
    ncontrol = 5
    Tcontrol = torch.linspace(
        torch.min(data[1]), torch.max(data[1]), ncontrol, device=device
    )
    ...
    def make(
        ...
        theta_vals,
        tau_vals,
        scale_functions=[lambda x: x] * 4,
        **kwargs
    ):
        n = temperature.PiecewiseScaling(
            Tcontrol, n_vals, values_scale_fn=scale_functions[0]
        )
        eta = temperature.PiecewiseScaling(
            Tcontrol, eta_vals, values_scale_fn=scale_functions[1]
        )
        theta = temperature.PiecewiseScaling(
            Tcontrol, theta_vals, values_scale_fn=scale_functions[2]
        )
        tau = temperature.PiecewiseScaling(
            Tcontrol, tau_vals, values_scale_fn=scale_functions[3]
        )
    ...
    scale_functions = [
        scaling.BoundedScalingFunction(
            torch.tensor(1.1, device=device), torch.tensor(15.0, device=device)
        ),
        scaling.BoundedScalingFunction(
            torch.tensor(20.0, device=device), torch.tensor(1000.0, device=device)
        ),
        scaling.BoundedScalingFunction(
            torch.tensor(200.0, device=device), torch.tensor(20000.0, device=device)
        ),
        scaling.BoundedScalingFunction(
            torch.tensor(10.0, device=device), torch.tensor(1000.0, device=device)
        ),
    ]
    loc_loc_priors = [
    ...
    test = optimize.DeterministicModel(
        lambda *args, **kwargs: make(
            *args, scale_functions=scale_functions, block_size=time_chunk_size, **kwargs
        ),
        names,
        loc_loc_priors,
    )
