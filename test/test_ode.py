...
class LogisticODE(torch.nn.Module):
  ...
  def forward(self, t, y):
    return self.r*(1.0-y/self.K) * y, (self.r - (2*self.r*y)/self.K)[:,:,None]

  def exact(self, t, y0):
    return (self.K*torch.exp(self.r*t) * y0[:,0] / (
      self.K + (torch.exp(self.r * t) - 1) * y0[:,0]))[...,None]
...
class TestBatchSimple(unittest.TestCase):
  def setUp(self):
    ...
    self.times = torch.tensor(np.array([np.linspace(0, 7.5, self.nsteps) for i in range(self.nbatch)]).T)
...
class FallingODE(torch.nn.Module):
  ...
  def exact(self, t, y0):
    return torch.stack((4.0*(t+torch.exp(-8.0*t)/8.0 - 1.0/8),
      4.0*(1-torch.exp(-8.0*t))), dim = 1)
...
class TestFallingBatch(unittest.TestCase):
  ...
  def test_forward(self):
    ...
    self.assertTrue(torch.max(torch.abs((numerical[1:]-exact[1:])/exact[1:])) < 1.0e-2)
  ...
  def test_backward(self):
    ...
    self.assertTrue(torch.max(torch.abs((numerical[1:]-exact[1:])/exact[1:])) < 1.0e-1)
...
def run_model(model, nbatch = 4, method = "forward-euler", nsteps = 10, substep = 2,
    solver_method = "diag"):
...
class TestGradient(unittest.TestCase):
  ...
  def test_grad_forward(self):
    ...
    with torch.no_grad():
      eps = 1.0e-6
      dmp = ((run_model(FallingParameterizedODE(self.m + self.m*eps, self.w, self.k), method = "forward-euler")-r0) / (eps*self.m)).numpy()
      dwp = ((run_model(FallingParameterizedODE(self.m, self.w * (1+eps), self.k), method = "forward-euler")-r0) / (eps*self.w)).numpy()
      dwk = ((run_model(FallingParameterizedODE(self.m, self.w, self.k * (1+eps)), method = "forward-euler")-r0) / (eps*self.k)).numpy()
  ...
  def test_grad_backward(self):
    ...
    with torch.no_grad():
      eps = 1.0e-6
      dmp = ((run_model(FallingParameterizedODE(self.m + self.m*eps, self.w, self.k), method = "backward-euler")-r0) / (eps*self.m)).numpy()
      dwp = ((run_model(FallingParameterizedODE(self.m, self.w * (1+eps), self.k), method = "backward-euler")-r0) / (eps*self.w)).numpy()
      dwk = ((run_model(FallingParameterizedODE(self.m, self.w, self.k * (1+eps)), method = "backward-euler")-r0) / (eps*self.k)).numpy()
  ...
  def test_grad_backward_lu(self):
    ...
    with torch.no_grad():
      eps = 1.0e-6
      dmp = ((run_model(FallingParameterizedODE(self.m + self.m*eps, self.w, self.k), method = "backward-euler",
        solver_method = "lu")-r0) / (eps*self.m)).numpy()
      dwp = ((run_model(FallingParameterizedODE(self.m, self.w * (1+eps), self.k), method = "backward-euler",
        solver_method = "lu")-r0) / (eps*self.w)).numpy()
      dwk = ((run_model(FallingParameterizedODE(self.m, self.w, self.k * (1+eps)), method = "backward-euler",
        solver_method = "lu")-r0) / (eps*self.k)).numpy()
